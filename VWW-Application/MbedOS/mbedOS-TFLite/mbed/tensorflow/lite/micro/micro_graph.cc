...
// #include "mbed.h"   // added by yujie for cycles recording
...
TfLiteStatus MicroGraph::InvokeSubgraph(int subgraph_idx) {
  ...
  const SubGraph* subgraph = (*subgraphs_)[subgraph_idx];
  // MicroPrintf("subgraph->operators()->size(): %d.", subgraph->operators()->size());
  // int latency_model = 0;
...
// only defined for builds with the error strings.
    ...
    // t1.stop();
    // MicroPrintf("OpNameFromRegistration(registration): %s, Invoke time: %d.", OpNameFromRegistration(registration), static_cast<u_long>(t1.read_high_resolution_us()));
    // latency_model += static_cast<u_long>(t1.read_high_resolution_us());
  ...
  current_subgraph_index_ = previous_subgraph_idx;
  // MicroPrintf("Model latency: %d.", latency_model);
  // MicroPrintf("\nOperators Logs:");
  // reinterpret_cast<MicroProfiler*>(context_->profiler)->Log();  // added by yujie
  // reinterpret_cast<MicroProfiler*>(context_->profiler)->ClearEvents();  // added by yujie
  return kTfLiteOk;
}
