...
#include "tensorflow/lite/micro/system_setup.h"  // edited by yujie for cycles recording test
...
uint32_t MicroProfiler::BeginEvent(const char* tag) {
  ...
  // start_timer();
  MicroPrintf("BeginEvent() currentTicks:%d.", start_ticks_[num_events_]);
...
void MicroProfiler::EndEvent(uint32_t event_handle) {
  ...
  MicroPrintf("EndEvent() currentTicks:%d.", end_ticks_[event_handle]);
  // MicroPrintf("EndEvent() Ticks:%d.", stop_timer());
}
...
void MicroProfiler::Log() const {
  ...
  for (int i = 0; i < num_events_; ++i) {
    ...
    MicroPrintf("%s took %d ticks (%d us).", tags_[i], ticks, TicksToUs(ticks));
  }
#endif
}
